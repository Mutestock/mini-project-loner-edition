...
from utils.config import STUDENTS_MIGRATIONS
...
class Student(AbstractEntity):
    ...
    def insert_query(self) -> None:
        conn = make_pg_pool()
        cursor = conn.cursor()
        query = f"INSERT INTO students (first_name, last_name, phone_number, email) VALUES ('{self.first_name}', '{self.last_name}','{self.phone_number}','{self.email}');"
